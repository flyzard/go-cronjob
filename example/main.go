...
func main() {
	...
	err := scheduler.AddJob("*/10 * * * * *", func() {
		fmt.Println("Task 1: Runs every 10 seconds -", time.Now().Format(time.RFC1123))
	})
	if err != nil {
	...
	// Add a job that runs at 9:00:00 AM every Monday
	err = scheduler.AddJob("0 0 9 * * Mon", func() {
		fmt.Println("Task 2: Runs at 9:00:00 AM every Monday -", time.Now().Format(time.RFC1123))
	})
	if err != nil {
	...
	// Add a job that runs every minute at 0 seconds
	err = scheduler.AddJob("0 * * * * *", func() {
		fmt.Println("Task 3: Runs every minute at 0 seconds -", time.Now().Format(time.RFC1123))
	})
	if err != nil {
	...
	err = scheduler.AddJob("0 0 0 1 * *", func() {
	...
	// Add a job with named month and day of week at specific time
	err = scheduler.AddJob("30 30 14 15 Jan-Mar Fri", func() {
		fmt.Println("Task 5: Runs at 14:30:30 on the 15th day of Jan, Feb, Mar and every Friday -", time.Now().Format(time.RFC1123))
	})
	if err != nil {
